...
public class TransactionFrequencyService {
    ...
    private List<RecurringBeneficiaryAlert> checkFrequency(List<Transaction> transactions, Object props) {
        ...
        if (props instanceof IndividualClientFrequencyProperties p) {
            ...
            typesAccountHolder = p.getTypesAccountHolder();
            ...
            throw new IllegalArgumentException("Unknown properties type");
        }
        ...
        List<Transaction> filtered = transactions.stream()
            .filter(t -> t.getDate().isAfter(windowStart))
            .filter(t -> allowedAccountTypes.contains(t.getDestinationAccount().getAccountType()))
            .toList();

        // Agrupar por (usuario, cuenta destino)
        record UserBeneficiary(Long userId, Long destinationAccountId) {}
